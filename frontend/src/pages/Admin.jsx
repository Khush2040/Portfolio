...
const Admin = () => {
    ...
    return (
        <PageWrapper className="min-h-screen py-32 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full bg-[#050505]">
            <motion.div
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8 }}
                className="mb-16 flex flex-col md:flex-row justify-between items-start md:items-end border-b border-white/10 pb-8 gap-6"
            >
                <div>
                    ...
                    <h3 className="text-4xl sm:text-5xl font-black tracking-tight text-white flex items-center gap-4">
                        <Database className="text-purple-400" size={40} />
                        Live Inbox
                    </h3>
                </div>

                <div className="glass px-6 py-3 rounded-xl border border-white/5 flex items-center gap-4">
                    <span className="relative flex h-3 w-3">
                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                        <span className="relative inline-flex rounded-full h-3 w-3 bg-green-500"></span>
                    </span>
                    <span className="text-sm font-bold tracking-widest uppercase text-gray-400">System Online</span>
                </div>
            </motion.div>
            ...
            {loading ? (
                ...
                <div className="glass p-16 rounded-3xl border border-white/5 flex flex-col items-center justify-center text-center">
                    ...
                    <h4 className="text-2xl font-bold text-gray-400 mb-2">Inbox is Empty</h4>
                ...
                >
                    {messages.map((msg, index) => (
                        <motion.div
                            ...
                            className="glass p-8 rounded-3xl border border-white/10 hover:border-white/20 transition-all duration-300 relative group overflow-hidden bg-gradient-to-br from-white/[0.02] to-transparent"
                        >
                            ...
                            <div className="flex items-center gap-3 mb-6 border-b border-white/5 pb-6">
                                ...
                                <div className="flex-1">
                                    <h4 className="text-lg font-bold text-white tracking-wide">{msg.name}</h4>
